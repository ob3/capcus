...
class Graph
{
	...
	public function __construct($base_uri='http://localhost:7474/')
	{
	...
	public function performPluginCommand($plugin, $command, $data, $inflate_nodes = true)
	{
		$uri = $this->base_uri.'ext/'.$plugin.'Plugin/Graphdb/'.$command;
                ...
                //extend for cypher query only (custom)
                $uri = $this->base_uri.strtolower($plugin);
...
class PropertyContainer
{
	...
	public function getProperties()
	{
        ...
        public function getRelationType(){
            $uri = $this->_neo_db->getBaseUri() .'relationship/types';
            list($response, $http_code) = HTTPUtil::jsonGetRequest($uri);
            return $response;
        }
}
...
class Node extends PropertyContainer
{
	...
	public function __construct(Graph $neo_db)
	{
		$this->_neo_db = $neo_db;
		$this->_is_new = TRUE;
                $this->_label = array();
		...
		if ($this->_is_new) 
		{
			...
			$this->_is_new=FALSE;
                ...
                if(!empty($this->_label)){
                    list($response, $http_code) = HTTPUtil::jsonPostRequest($this->getUri().'/labels', $this->_label);
                    if ($http_code!=204) throw new HttpException($http_code);
                }
	}
	...
	{
            $key = array_search($label, $this->_label);
            if($key)
                unset($this->_label[$key]);
	}
        
        public function getLabel(){
            if(empty($this->_label)){
                $uri = $this->getUri().'/labels';
                list($response, $http_code) = HTTPUtil::jsonGetRequest($uri);
                $this->_label=array_merge($this->_label, $response);                
            }
            return $this->_label;
        }
	...
	public static function inflateFromResponse(Graph $neo_db, array $response)
	{
...
class Relationship extends PropertyContainer
{
	...
	public function __construct(Graph $neo_db, Node $start_node, Node $end_node, $type)
	{
		$this->_neo_db = $neo_db;
		$this->_is_new = TRUE;
		$this->_type = $type;
